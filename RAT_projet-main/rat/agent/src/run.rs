...
pub fn run(api_client: &ureq::Agent, conf: config::Config, agent_id: uuid::Uuid) -> ! {
    let sleep_for = Duration::from_secs(1);
    let get_job_route = format!("{}/api/agents/{}/job", conf.server_url, agent_id);
    let post_job_result_route = format!("{}/api/jobs/result", conf.server_url);
    ...
    loop {
        ...
        let (job_id, job) = match decrypt_and_verify_job(&conf, agent_id, encrypted_job) {
            Ok(res) => res,
            Err(err) => {
...
fn decrypt_and_verify_job(
    ...
    buffer_to_verify.append(&mut agent_id.as_bytes().to_vec());
